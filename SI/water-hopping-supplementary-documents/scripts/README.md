...
- Equilibration: update `example.yaml` with input files and specify `nstepsMD`. In `example.py`, uncomment lines pertaining to minimization and MD. Use `example.pbs`to execute `example.py` on a cluster (TSCC in this case).
